...
- 輪紋病（`anthracnose` / Botryosphaeria dothidea）・赤星病（`largePatch` / Gymnosporangium yamadae）の生理モデルを追加し、5病害すべて 0–100% 表示に対応
- 判定ロジックモーダルの説明文・計算式を更新（赤星病は中間宿主ビャクシン類の有無は未考慮）

**PGR適時・発芽予測 / 収穫適時積算温度**

- 摘果適時（DAFB）ゲージの目盛ラベルを「摘花開始(20D)」「摘花終了(40D/60D)」に変更
- 収穫適時積算温度ゲージに「収穫目安」「収穫終了」の2行ラベルと数値を目盛位置に表示

**AI質問箱**

- 回答表示時、長文でも**回答の先頭**が見える位置へスクロールするよう調整（末尾固定スクロールを廃止）
