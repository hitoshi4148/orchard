const WorkMemoUI = (() => {
  const AUTH_STORAGE_KEY = "orchardWorkMemoAuth";
  const SHEET_STORAGE_PREFIX = "orchardWorkMemoSheetId:";
  ...
  async function loadRows() {
    ...
    rows = (data.values || [])
      ...
      .filter((row) => row.date || row.fieldName || row.memo);
  ...
  async function updateRow(sheetRow, date, fieldName, memo) {
    const sheetId = await ensureSpreadsheetId();
  ...
  async function appendRow(date, fieldName, memo) {
    const sheetId = await ensureSpreadsheetId();
    await sheetsFetch(
  ...
  function buildWorkMemoInfoHtml(sheetTitle) {
    ...
    return `<span class="series-info-anchor work-memo-info-anchor">
      <button type="button" class="series-info-btn" aria-label="果実園しごとノートの保存先について">i</button>
      <span class="series-info-popover work-memo-info-popover" role="tooltip">
        ここで入力した内容は、各利用者の Google ドライブ（マイドライブ）に「${fileName}」というスプレッドシート（日付・圃場名・メモ）として保存されます。
        果実園しごとポータルのサーバー側ではデータを保持・管理しません。保存先は連携した Google アカウント内のみです。
        <br><br>
        <strong>本機能は現在、モニター限定の試験運用中です。</strong>
        参加をご希望の方は、Gmail から hitoshi.yoshinobu@gmail.com 宛に「試験参加希望」とお送りください。名前・住所等の個人情報は一切不要です。
      </span>
    </span>`;
  }
  ...
  function buildWorkMemoTitleHtml(sheetTitle) {
    return `<div class="work-memo-title-main">
      <h3 class="disease-risk-title">果実園しごとノート</h3>
  ...
  function buildFormHtml(today) {
    ...
    const formFieldName = editing && selectedRow ? selectedRow.fieldName : "";
    ...
    if (editing) {
      return `<form class="work-memo-form work-memo-form--edit" id="work-memo-form">
        ...
        <input type="text" id="work-memo-field-input" placeholder="圃場名" maxlength="120" aria-label="圃場名" value="${escapeHtml(formFieldName)}">
    ...
    return `<form class="work-memo-form" id="work-memo-form">
      <input type="date" id="work-memo-date" value="${escapeHtml(formDate)}" aria-label="日付">
      <input type="text" id="work-memo-field-input" placeholder="圃場名" maxlength="120" aria-label="圃場名">
    ...
    container.innerHTML = `<div class="work-memo-panel disease-risk-panel">
      ...
      <div class="work-memo-table-wrap" id="work-memo-scroll">
        <table class="work-memo-table">
          <thead>
            <tr><th>日付</th><th>圃場名</th><th>メモ</th></tr>
          </thead>
  ...
  function readFormValues() {
    ...
    const fieldInput = document.getElementById("work-memo-field-input");
  ...
  async function handleSubmit(event) {
    ...
    const { date, fieldName, memo } = readFormValues();
    ...
    try {
      if (editing) {
        await updateRow(selectedSheetRow, date, fieldName || "", memo || "");
        clearSelection();
        await loadRows();
        setStatus("更新しました");
      } else {
        await appendRow(date, fieldName || "", memo || "");
        const fieldInput = document.getElementById("work-memo-field-input");
        const memoInput = document.getElementById("work-memo-text");
